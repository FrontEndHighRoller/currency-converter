...
export default function App() {
  ...
  const [realTimeExchangeRate, setRealTimeExchangeRate] = useState({});

  const apiKey = "a8ed74d95e57fda122ade78b";

  useEffect(() => {
async function fetchExchangeRates() {
  try {
    const response = await fetch(
      `https://v6.exchangerate-api.com/v6/${apiKey}/latest/${startCurrency}`
    );
    const data = await response.json();
    if (data.result === "success") {
      setRealTimeExchangeRate(data.conversion_rates);
    } else {
...
fetchExchangeRates();}, [startCurrency]

  )


  const convertedAmount = realTimeExchangeRate[targetCurrency]
    ? currencyInput * realTimeExchangeRate[targetCurrency]
    : 0;
...
>
  ...
  <label htmlFor="currencyInput">Start Currency to Target Currency Conversion</label>
  <input id="currencyInput"
         ...
         style={{
            ...
            borderRadius: "10px",
            padding: "0.5rem",
            fontSize: "1rem", // important for mobile zoom
          }}
          value={currencyInput}
          min="1"
  ...
  >
    ...
    <option value="ZWL">ZWL – Zimbabwe Dollar (Old)</option>
  ...
  <select
    id="targetCurrency"
    style={{borderRadius: "10px", padding: "0.3125rem"}}
